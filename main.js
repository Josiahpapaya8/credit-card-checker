...
const validateCred = (array) => {
    let luhnArray = array;
    if (luhnArray.length % 2 === 0){
        luhnArray = luhnArray.map((num, idx) => {
            if (idx % 2 === 0){
                if (num * 2 > 9){
                    return (num * 2) - 9;
                } else {
    ...
    let sum = luhnArray.reduce(function(a,b){
        return a + b;
    });
    if (sum % 10 === 0){
        return true;
    ...
    for (let i = 0; i < invalidCards.length; i++){
        for (let j = 0; j < 1; j++){
            if (array[i][j] === 3){
                ...
                if (invalidCardCompanies.indexOf('Master Card') === -1){
                    invalidCardCompanies.push('Master Card');
                }
            } else if (array[i][j] === 6){
